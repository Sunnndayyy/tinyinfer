...
ARTIFACT_DIR = Path(".tinyinfer/roofline")
...
def clean_artifacts() -> None:
    """Remove only the generated Roofline artifacts."""
    _remove(ARTIFACT_DIR)


def run_default(*, capture: bool = False) -> int:
    """Run the opinionated benchmark or replace the four GPU captures."""
    if capture:
        capture_dir = ARTIFACT_DIR / "captures"
        previous_dir = ARTIFACT_DIR / ".captures.previous"
        _remove(previous_dir)
        if capture_dir.exists():
            capture_dir.rename(previous_dir)
        try:
            result = main(["--metal-capture-dir", str(capture_dir)])
        except BaseException:
            _remove(capture_dir)
            if previous_dir.exists():
                previous_dir.rename(capture_dir)
            raise
        if result == 0:
            _remove(previous_dir)
        else:
    ...
    arguments = [
        ...
        "--plot",
        str(ARTIFACT_DIR / "roofline.svg"),
    ]
    counters = ARTIFACT_DIR / "counters.json"
    if counters.exists():
        arguments.extend(("--counters", str(counters)))
...
def write_roofline_svg(
    ...
    *,
    ...
    lines = [
        ...
        "<style>text{font-family:-apple-system,BlinkMacSystemFont,sans-serif;fill:#202124}.grid{stroke:#d9dde3;stroke-width:1}.axis{stroke:#59636e;stroke-width:1.5}.ref{fill:none;stroke:#777;stroke-width:2}.series{fill:none;stroke-width:2}.label{font-size:13px}.small{font-size:12px}</style>",
    ...
    for weight_format, color in colors.items():
        series = sorted(
            (result for result in measurements if result.weight_format == weight_format),
            key=lambda result: result.rows,
        )
        if len(series) > 1:
            points = " ".join(
                f"{x_position(result.counts.arithmetic_intensity):.1f},"
                f"{y_position(result.achieved_tflops):.1f}"
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    for rows in args.rows:
        ...
        q8_ms, bf16_ms = measure_mps_pair(
            q8_operation,
            bf16_operation,
            warmup=args.warmup,
