...
RESULT_IDENTITY_FIELDS = ("model", "hardware", "device", "dtype", "profile", "kv_cache")
...
def load_records(path: Path) -> list[dict[str, Any]]:
    ...
    if not isinstance(records, list) or any(
        not isinstance(record, dict) or not set(RESULT_FIELDS) <= record.keys()
...
def format_leaderboard(records: list[dict[str, Any]]) -> str:
    ...
    lines = [
        ...
        "",
        "Generated by `tinyinfer leaderboard` from local aggregate results. Compare rows only within the same model, hardware, dtype, and profile.",
        "",
        "| Model | Hardware | Profile | KV cache | TTFT p50 | Decode tok/s | Change |",
        "| --- | --- | --- | --- | ---: | ---: | ---: |",
    ]
    ...
    for record in sorted(
        ...
        lines.append(
            f"| {record['model']} | {hardware} | {record['profile']} | {record['kv_cache']} | "
