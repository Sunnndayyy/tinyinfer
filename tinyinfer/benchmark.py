...
COMPARISON_GROUP_FIELDS = tuple(field for field in RESULT_IDENTITY_FIELDS if field != "kv_cache")
WEIGHT_COMPARISON_GROUP_FIELDS = tuple(
    field for field in RESULT_IDENTITY_FIELDS if field != "quantization"
)
RESULT_FIELDS = (*RESULT_IDENTITY_FIELDS, "ttft_seconds", "decode_tokens_per_second")
LEGACY_RESULT_FIELDS = set(RESULT_FIELDS) - {
    "attention",
    "decoding",
    "quantization",
    "revision",
...
def format_summary(result: dict[str, Any]) -> str:
    ...
    return "\n".join(
        [
            ...
            f"weights: {metadata.get('quantization', 'none')}",
...
def save_leaderboard_result(result: dict[str, Any], path: Path = LOCAL_RESULTS) -> dict[str, Any]:
    ...
    record = {
        ...
        "attention": metadata.get("attention", "unknown"),
...
def load_records(path: Path) -> list[dict[str, Any]]:
    ...
    if not isinstance(records, list) or any(
        not isinstance(record, dict) or not LEGACY_RESULT_FIELDS <= record.keys()
        for record in records
    ):
        raise ValueError(f"invalid benchmark data in {path}; delete it and rerun")
    for record in records:
        record.setdefault("decoding", "autoregressive")
        record.setdefault("attention", "unknown")
        record.setdefault("quantization", "none")
        record.setdefault("revision", None)
        record.setdefault("artifact_path", None)
    return records
...
def format_leaderboard(records: list[dict[str, Any]]) -> str:
    ...
    for record in records:
        ...
        if record["kv_cache"] == "none":
            cache_baselines[group] = record["decode_tokens_per_second"]
        if record["quantization"] == "none":
            weight_group = record_key(record, WEIGHT_COMPARISON_GROUP_FIELDS)
            weight_baselines[weight_group] = record["decode_tokens_per_second"]
    ...
    for record in sorted(
        ...
        group = record_key(record, COMPARISON_GROUP_FIELDS)
        baseline = cache_baselines.get(group)
        if record["kv_cache"] == "none":
            cache_change = "baseline"
        elif baseline:
            cache_change = f"{(record['decode_tokens_per_second'] / baseline - 1) * 100:+.1f}%"
        else:
            cache_change = "—"
        weight_group = record_key(record, WEIGHT_COMPARISON_GROUP_FIELDS)
        weight_baseline = weight_baselines.get(weight_group)
        if record["quantization"] == "none":
            weight_change = "baseline"
        elif weight_baseline:
            weight_change = (
                f"{(record['decode_tokens_per_second'] / weight_baseline - 1) * 100:+.1f}%"
            )
        else:
            weight_change = "—"
        revision = str(record["revision"])[:12] if record["revision"] else "unknown"
        hardware = f"{record['hardware']} · {record['device']}/{str(record['dtype']).removeprefix('torch.')}"
        lines.append(
            f"| {record['model']} | {revision} | {hardware} | {record['profile']} | "
            f"{record['decoding']} | "
            f"{record['attention']} | "
            f"{record['quantization']} | "
            f"{record['kv_cache']} | "
            f"{record['ttft_seconds']:.3f}s | {record['decode_tokens_per_second']:.1f} | "
            f"{cache_change} | {weight_change} |"
        )
    return "\n".join(lines) + "\n"
