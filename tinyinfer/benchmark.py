...
def load_records(path: Path) -> list[dict[str, Any]]:
    ...
    legacy_fields = set(RESULT_FIELDS) - {"attention"}
    if not isinstance(records, list) or any(
        not isinstance(record, dict) or not legacy_fields <= record.keys() for record in records
    ):
...
def format_leaderboard(records: list[dict[str, Any]]) -> str:
    ...
    lines = [
        ...
        "",
        "Generated by `tinyinfer leaderboard` from local aggregate results. Compare rows only within the same model, hardware, dtype, profile, and attention implementation.",
        "",
        "| Model | Hardware | Profile | Attention | KV cache | TTFT p50 | Decode tok/s | Change |",
        "| --- | --- | --- | --- | --- | ---: | ---: | ---: |",
    ]
    ...
    for record in sorted(
        ...
        lines.append(
            f"| {record['model']} | {hardware} | {record['profile']} | {record['attention']} | "
            f"{record['kv_cache']} | "
